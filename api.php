...
function doValidate($email, $password, $proxy) {
    ...
    for ($attempt = 0; $attempt < 5; $attempt++) {
        ...
        // Metodo 1: IMAP direto (SEM proxy) — metodo principal
        ...
        // Metodo 2: Zimbra SOAP via proxy (se IMAP direto falhou)
...
function tryZimbraSoap($email, $password, $timeout, $proxy) {
    ...
    // 403 = Akamai CDN bloqueando, NAO e credencial invalida
    if ($code === 403 || $code === 401) return null;

    // HTML em vez de XML = CDN block, nao e resposta do Zimbra
    ...
    // SOAP fault com auth failed = credencial invalida real
